...
<!-- Форма поиска -->
    ...
    <form id="serch_teacher" action="vendor/serch_teacher.php" method="post">
...
<?php
    ...
    $serch_works = mysqli_query($connect, "SELECT accepted_users.id,accepted_users.full_name, groups_stud.name_group, direction.name_dir FROM(( `accepted_users` INNER JOIN `groups_stud` ON groups_stud.id = accepted_users.group_user) INNER JOIN `direction` ON direction.id = groups_stud.Direction_id)");
